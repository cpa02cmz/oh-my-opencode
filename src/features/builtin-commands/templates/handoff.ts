...
# PHASE 1: READ SESSION HISTORY

Use the session_read tool to retrieve the current session's message history:

\`\`\`
session_read({ session_id: "<current_session_id>" })
\`\`\`
...
Begin by reading the current session to extract the handoff context.
`
